...
export interface UseOrdersApiInterface {
    confirmOrder(orderId: number, table: number | null, isTakeawayOrKiosk: boolean): Promise<BaseResponse>;
    addOrder(customer: string, guests: number | null, isTakeAway: boolean, isTakeAwayKiosk: boolean, table: number | null, isVoucher: boolean, parentOrder: number | null, products: CreateOrderProduct[], menus: CreateOrderMenu[]): Promise<CreateOrderResponse>;
    ...
    serveOrder(orderId: number): Promise<BaseResponse>;
}
